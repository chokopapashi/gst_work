...
int main(int argc, char *argv[]) {
	...
	if(!data.pipeline || !data.source || !data.convert || !data.resample || !data.sink) {
		g_printerr("Not all elements cound be created : %p, %p, %p, %p, %p\n",
		...
		if(msg != NULL) {
			...
			switch(GST_MESSAGE_TYPE(msg)) {
				...
				case GST_MESSAGE_EOS:
					g_print("End-Of-Straem reached.\n");
					terminate = TRUE;
					break;
				case GST_MESSAGE_STATE_CHANGED:
					/* We are only interested in state-changed messages from the pipline */
...
exit:
	/* Unreference the new pa8s caps, if we got them */
